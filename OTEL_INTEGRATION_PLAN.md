...
The `~/alpine-otel` project integrates SONiC-Alpine (a container-native SONiC variant running on KNE/Kubernetes with the Lemming/Lucius dataplane) with OpenTelemetry for full-stack network observability. The current state has a **partially wired** OTel collector (`docker-sonic-otel`) that only accepts OTLP and dumps to debug — no gNMI receiver, no real backend exporters, and no per-container telemetry coverage. This plan delivers end-to-end observability for **all 10 SONiC containers + CLI**, from Redis DB counters through gNMI subscriptions to Prometheus/Jaeger/GCP backends.

---

## Executive Critique & Architecture Improvements
...
## Per-Container Telemetry Coverage Matrix

### 1. database (redis-server)
...
### 2. swss (portsyncd, intfsyncd, neighsyncd, orchagent, intfmgrd, vlanmgrd)
...
### 3. syncd (syncd, SAI API, ASIC SDK)
...
### 4. bgp (bgpd/GoBGP, zebra/sysrib, fpmsyncd)
...
### 5. lldp (lldpd, lldpmgrd, lldp_syncd)
...
### 10. CLI / Config (CLI, sonic-cfggen)
...
## Phase 1: Verify Data Path (gnmic validation)
...
receivers:
  ...
  gnmi:
    ...
    subscriptions:
      ...
      bgp_global:
        ...
        mode: sample
        sample_interval: 10s
      # LLDP neighbors (lldp: lldpd, lldp_syncd)
      lldp_neighbors:
        path: /openconfig-lldp:lldp/interfaces/interface[name=*]/neighbors/neighbor[id=*]/state
        mode: on_change
      # LLDP counters
      lldp_counters:
        path: /openconfig-lldp:lldp/interfaces/interface[name=*]/state/counters
        mode: sample
        sample_interval: 10s
      # LAG state (teamd: teamd, teamsyncd)
      lag_state:
        path: /openconfig-if-aggregate:interfaces/interface[name=*]/aggregation/state
        mode: on_change
      # Platform components (pmon: sensord, fancontrol — virtual returns empty)
      platform_state:
        path: /openconfig-platform:components/component[name=*]/state
        mode: sample
        sample_interval: 30s
      # CRM resource monitoring (syncd/orchagent)
      crm_stats:
        path: /sonic-crm:sonic-crm/CRM_STATS
        mode: sample
        sample_interval: 30s

  # --- Layer A: OTLP Receiver (for Lemming/Lucius native OTel SDK) ---
  otlp:
    protocols:
      grpc:
  ...
  # --- Layer B: Host Metrics (per-container process monitoring) ---
  hostmetrics:
  ...
  # --- Layer B: File Log Receiver (container logs) ---
  filelog/syslog:
    include:
      - /var/log/syslog
    start_at: end
    operators:
      - type: regex_parser
        regex: '^(?P<timestamp>\w+\s+\d+\s+[\d:]+)\s+(?P<hostname>\S+)\s+(?P<process>[^\[]+)\[(?P<pid>\d+)\]:\s+(?P<message>.*)$'
        timestamp:
...
## Phase 3: COUNTERS_DB → YANG/OpenConfig Mapping Definitions
...
## Phase 4: Packaging & Boot-Time Reliability

### 4.1 supervisord.conf Changes
...
## Phase 5: Backend Integration & Visualization

### 5.1 Central Backend Stack (Kubernetes Manifests)
...
- Pre-built dashboards (see 5.3)

### 5.2 TLS Configuration (Production)
...
### 6.2 Acceptance Criteria for "Self-Verifying Network Simulations"
...
## Improvements & Alternatives Comparison
