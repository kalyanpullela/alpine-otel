...
*******************************************************************************/
/*
 * T100 TODO: This file is cloned from V2(TL10).
 * The debug header format may be different for V4(T100).
 * Revisit once the T100 debug header available.
 */
...
#include "ipd.h"
#include "ldh_structs_ipd_v4.h"
#include "inno_ldh.h"
#include "inno_enet.h"
#include "inno_cmipd_v4.h"

typedef struct ldh_v4_s {
    union {
...
int
inno_unpack_ldh_header_v4(uint8_t    *buf,
                          inno_ldh_t *ldh_hdr)
{
	inno_ldh_v4_t *ldh = (inno_ldh_v4_t  *) ldh_hdr;
...
int
inno_vf2_queue_set_v4(uint8_t *buf,
                      uint8_t queue)
{
    inno_ldh_v4_t ldh;
...
bool
inno_debug_hdr_present_v4(inno_ldh_t *ldh_hdr)
{
	inno_ldh_v4_t *ldh = (inno_ldh_v4_t  *) ldh_hdr;
                         ...
                         uint16_t  *intf)
{
    ...
    inno_unpack_cmhdr_v4(buf, &ipdfld);
...
void
inno_populate_inno_hdr_v4(inno_ldh_t         *ldh_hdr,
                          inno_info_header_t *ih,
						  inno_dma_alloc_t  *dma,
 ...
 */
int
inno_unpack_ext_hdrs_v4(inno_device_t *idev,
                        inno_dma_alloc_t *dma,
                        inno_ldh_t *ldh_hdr,
